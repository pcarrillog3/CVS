...
if ($mysqli->connect_error) {
  ...
  echo "fail";
        ...
        $query = $mysqli->query("select * from almacenes where idalmacenes=".$id);
        
        if ($query->num_rows > 0) {
  // output data of each row
   $row=$query->fetch_array(MYSQLI_NUM);
   echo "$('#NOMBRE').val('".$row[1]."');";
   echo "$('#RAZON').val('".$row[3]."');";
...
if ($mysqli->connect_error) {
 ...
 echo '<td>'.$row[0].'</td>';
  echo '<td>'.$row[1].'</td>';
  echo '<td><div class="informacion">'.$row[3].'</div></td>';
...
<!--            <td>
            ...
            *P.P=Precio Partida
                    ...
                    <div class="col-sm-6" style=" border-style:solid; border-color:transparent ; border-left-color:#ccccff">
                        ...
                        <form action="FILES/edit_almacen.php" method="post">
